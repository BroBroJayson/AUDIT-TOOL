...
<div class="unmatch-section" id="unmatchDrSalesSection">
    <h3 class="text-center mb-4">UNMATCH DR-DRRCV</h3>

    <div class="mt-4 text-left">
        <a href="{{ url('/export-unmatchedDrSales') }}" class="btn btn-outline-success">
    ...
    <div class="table-responsive" style="max-height: 600px; overflow-y: auto;">
        <table class="table table-striped table-hover" id="comparisonTable">
            <thead class="text-black">
                <tr>
                    <th class="table-info" style="position: sticky; top: 0;">No.</th>
                    ...
                    <th class="table-info" style="position: sticky; top: 0;">SOURCE</th>
                </tr>
            </thead>
            <tbody>
                @foreach($file1Data as $index => $row)
                    @if(!in_array($row[6], array_column($file2Data, 9)))
                ...
                @foreach($file2Data as $index => $row)
                    @if(!in_array($row[9], array_column($file1Data, 6)))
                        <tr class="unmatched-row">
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $row[9] }}</td>
                            <td>{{ $row[10] }}</td>
